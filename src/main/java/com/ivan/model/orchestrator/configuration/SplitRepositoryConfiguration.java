...
public class SplitRepositoryConfiguration implements BeanFactoryAware {
    ...
    public void onPostConstruct() {
        ...
        for (Class repoClass : repoClasses) {
            List<Class> genericTypes = Arrays.stream(Arrays.stream(repoClass.getGenericInterfaces())
            ...
            var splitAnnotation = ((SplitEntity) genericTypes.get(0).getAnnotation(SplitEntity.class));
            var entity = splitAnnotation.entity();
            var entityHandlerRepository = splitAnnotation.entityHandlerRepository();
